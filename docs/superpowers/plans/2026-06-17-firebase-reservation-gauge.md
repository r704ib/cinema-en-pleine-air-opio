...
**Architecture:** Visitors write reservations directly to Firestore from the browser (Firebase JS SDK), inside a transaction that atomically caps the shared gauge at 150 places. Firestore security rules enforce data validity and the cap. Two Cloud Functions, triggered by Firestore writes, send transactional emails via the Brevo API. A new static page (`annuler.html`) lets a visitor cancel their own reservation using the secret document ID from their confirmation email.
...
## Task 3: Règles de sécurité Firestore (TDD avec l'émulateur)
...
- Produces: un fichier `firestore.rules` validé par tests, garantissant que `meta/gauge.reserved` reste toujours entre 0 et 150, qu'une réservation valide ne dépasse jamais 10 places, et que le champ honeypot `hp` doit être vide. Les Tâches 5 et 6 écrivent dans Firestore en s'appuyant sur ces règles.
...
Expected: tous les tests `assertSucceeds(...)` échouent (puisque les règles temporaires bloquent tout). Tests `assertFails(...)` passent déjà (tout est bloqué par défaut). Au global, la commande sort en erreur (`Tests: ... failed`).
...
git commit -m "feat: règles de sécurité Firestore pour les réservations et la jauge"
```

---

## Task 4: Cloud Functions — contenu des emails et orchestration
...
**Interfaces:**
- Consumes: le document `reservations/{id}` tel que défini en Tâche 3 ; la clé API Brevo et l'adresse vérifiée `Oria.ei@outlook.fr` de la Tâche 1 ; l'URL de production `https://cinema-en-pleine-air-opio.netlify.app`.
- Produces: deux Cloud Functions déployées (`onReservationCreated`, `onReservationCancelled`) qui envoient les emails de confirmation/notification. Aucune autre tâche ne dépend directement de leur code, seulement de leur effet (réception d'email), vérifié manuellement en Tâche 7.
...
- [ ] **Step 3: Écrire les tests du contenu des emails (ils doivent échouer, le module n'existe pas encore)**
...
- [ ] **Step 5: Implémenter `functions/email-content.js`**
...
- [ ] **Step 7: Implémenter l'orchestration `functions/index.js`**
...
- [ ] **Step 8: Configurer le secret Brevo et déployer**
...
Expected: sortie se terminant par `✔  Deploy complete!`, avec les deux fonctions `onReservationCreated` et `onReservationCancelled` listées comme déployées.

- [ ] **Step 9: Vérification manuelle du déclenchement**

Dans la console Firebase → Firestore → collection `reservations` → ajouter manuellement un document avec tous les champs valides (voir `validReservation()` de la Tâche 3 comme modèle) et `status: "active"`.
...
```bash
git add functions/package.json functions/email-content.js functions/test/email-content.test.js functions/index.js functions/package-lock.json
git commit -m "feat: cloud functions d'envoi d'email via Brevo à la réservation et l'annulation"
```
...
**Interfaces:**
- Consumes: le schéma de données et les règles Firestore de la Tâche 3 ; l'objet `firebaseConfig` à 6 champs obtenu à la Tâche 1, Étape 4.
- Produces: un formulaire de réservation qui écrit dans Firestore et une jauge alimentée par `onSnapshot` sur `meta/gauge`, réutilisés nulle part ailleurs (page autonome).
...
<script type="module">
  ...
  import {
    getFirestore, doc, collection, runTransaction, onSnapshot, serverTimestamp
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      var reservation = {
        ...
        totalPlaces: totalRequested(),
        montantEstime: qty.adultes * PRICE_ADULTE + qty.enfants * PRICE_ENFANT,
        status: 'active',
      ...
      var gaugeRef = doc(db, 'meta', 'gauge');
      var reservationRef = doc(collection(db, 'reservations'));

      runTransaction(db, async function (tx) {
        var gaugeSnap = await tx.get(gaugeRef);
        var reserved = gaugeSnap.data().reserved;
        if (reserved + reservation.totalPlaces > MAX_PLACES) {
          throw new Error('FULL');
        }
        tx.set(reservationRef, reservation);
        tx.update(gaugeRef, { reserved: reserved + reservation.totalPlaces, updatedAt: serverTimestamp() });
      }).then(function () {
        form.style.display = 'none';
...
Temporarily add this line right after `var db = getFirestore(app);` for local testing only:
...
(add `connectFirestoreEmulator` to the existing `firebase-firestore.js` import list instead of a separate import line)
...
Une fois la vérification faite, **retirer** la ligne `connectFirestoreEmulator(...)` et l'import correspondant avant de continuer (ce code ne doit pas partir en production).
...
**Interfaces:**
- Consumes: le schéma `reservations/{id}` et les règles de la Tâche 3 ; l'objet `firebaseConfig` de la Tâche 1 ; les liens `https://cinema-en-pleine-air-opio.netlify.app/annuler.html?id=<id>` générés par `buildCancelUrl` (Tâche 4).
...
<script type="module">
  import { initializeApp } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-app.js";
  import {
    getFirestore, doc, runTransaction, getDoc, serverTimestamp
  } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";
  ...
  if (cancelBtn) {
    cancelBtn.addEventListener('click', function () {
      ...
      runTransaction(db, async function (tx) {
        var reservationSnap = await tx.get(reservationRef);
        if (!reservationSnap.exists() || reservationSnap.data().status !== 'active') {
          throw new Error('ALREADY_CANCELLED');
        }
        var gaugeSnap = await tx.get(gaugeRef);
        var totalPlaces = reservationSnap.data().totalPlaces;
        tx.update(reservationRef, { status: 'cancelled', cancelledAt: serverTimestamp() });
        tx.update(gaugeRef, { reserved: gaugeSnap.data().reserved - totalPlaces, updatedAt: serverTimestamp() });
      }).then(function () {
        confirmState.style.display = 'none';
...
Avec le même émulateur Firestore lancé qu'à la Tâche 5, ajouter temporairement la ligne `connectFirestoreEmulator(db, '127.0.0.1', 8080)` (même procédure qu'à la Tâche 5, Étape 5) dans `annuler.html`.
...
Retirer ensuite la ligne `connectFirestoreEmulator(...)` et son import avant de continuer (ne doit pas partir en production).
